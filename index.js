...
(async () => {
  ...
  await page.waitForSelector("._audio_page_layout.audio_page_layout", {
    timeout: 0
  });
  ...
  let nameList = await page.evaluate(() => {
    ...
    let audioList = Array.from(
      document.querySelector(
        "div.audio_page__audio_rows_list._audio_page__audio_rows_list._audio_pl.audio_w_covers.audio_owner_list_canedit"
      ).childNodes
  ...
  var interceptorCallback = async req => {
    ...
    if (new URL(req.url()).host.includes("vkuseraudio.net")) {
      let musicType = new URL(req.url()).pathname.includes(".mp3")
        ? { ext: ".mp3" }
        : new URL(req.url()).pathname.includes(".m3u")
        ? { ext: ".m3u" }
        : null;
      ...
      if (musicType) {
        await download({
          link: req.url(),
          fname: nameList[position++] + musicType.ext
        });

        let audioRemains = await page.evaluate(() => {
